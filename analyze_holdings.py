...
class HoldingsAnalyzer:
    ...
    def load_data(self) -> None:
        ...
        if history_file.exists():
            with open(history_file) as f:
                ...
                # Extract percentage_change from action column if not present
                if (
                    "percentage_change" not in self.history_df.columns
                    and "action" in self.history_df.columns
                ):
                    self.history_df["percentage_change"] = self.history_df[
                        "action"
                    ].apply(self._extract_percentage_change)
    ...
    def _merge_enriched_data(self) -> None:
        ...
        if (
            self.holdings_df is not None
            and self.stocks_df is not None
            and not self.holdings_df.empty
            and not self.stocks_df.empty
        ):
            # Merge enriched stock data into holdings
            enriched_columns = [
                "sector",
            ...
            available_columns = [
                col for col in enriched_columns if col in self.stocks_df.columns
            ]
            ...
            if available_columns:
                ...
                self.holdings_df = self.holdings_df.merge(
                    merge_data, on="ticker", how="left"
                )
                logging.info(
                    f"Merged enriched data for {len(available_columns)} fields: {', '.join(available_columns)}"
                )
            else:
    ...
    def _extract_percentage_change(self, action: str) -> Optional[float]:
        ...
        if "Reduce" in action_str:
            # Try to extract number even without % sign
            match = re.search(r"Reduce\s+([+-]?\d+\.?\d*)", action_str)
            if match:
                value = float(match.group(1))
                # Ensure it's negative for reductions
                return -abs(value)
        ...
        if self.managers_df is not None:
            for _, row in self.managers_df.iterrows():
                ...
                if (
                    name
                    and name != manager_id
                    and manager_id not in self.manager_name_map
                ):
    ...
    def analyze_top_holdings(self) -> pd.DataFrame:
        ...
        missing_columns = [
            col for col in required_columns if col not in self.holdings_df.columns
        ]
        if missing_columns:
            logging.warning(
                f"Missing required columns for top holdings analysis: {missing_columns}"
            )
            return pd.DataFrame()
    ...
    def analyze_historical_manager_performance(self) -> pd.DataFrame:
        ...
        # Sort by total value
        historical_managers = historical_managers.sort_values(
    ...
    def analyze_value_opportunities(self) -> pd.DataFrame:
        ...
        if missing_columns:
            logging.debug(
                f"Missing columns for analyze_value_opportunities: {missing_columns}"
            )
            # Return empty DataFrame if essential columns are missing
            return pd.DataFrame()
    ...
    def analyze_high_conviction_stocks(self) -> pd.DataFrame:
        ...
        if self.history_df is not None:
            ...
            first_buys = (
                self.history_df[self.history_df["action_type"] == "Buy"]
                .groupby(["ticker", "manager_id"])["period"]
                .first()
                .reset_index()
            )
        ...
        if "managers" in grouped.columns:
            grouped["manager_names"] = grouped["managers"].apply(
                lambda x: ", ".join(
                    [
                        self.manager_name_map.get(m.strip(), m.strip())
                        for m in x.split(",")
                    ]
                )
            )
    ...
    def analyze_interesting_stocks_overview(self) -> pd.DataFrame:
        """Check if required columns exist."""
        required_columns = ["market_cap", "pe_ratio", "sector"]
        if self.stocks_df is None or self.stocks_df.empty:
            return pd.DataFrame()

        missing_columns = [
            col for col in required_columns if col not in self.stocks_df.columns
        ]
        if missing_columns:
            logging.debug(
                f"Missing columns for analyze_interesting_stocks_overview: {missing_columns}"
            )
            # Return empty DataFrame if essential columns are missing
            return pd.DataFrame()
        ...
        if self.history_df is not None:
            recent_activity = (
                self.history_df[self.history_df["action_type"].isin(["Buy", "Add"])]
                .groupby("ticker")
        ...
        if "last_buy_period" in overview.columns:
            overview.loc[
                overview["last_buy_period"] == "Q4 2024", "investment_timing"
            ] = "Good"
        ...
        overview["manager_names"] = overview["managers"].apply(
            lambda x: ", ".join(
                [self.manager_name_map.get(m.strip(), m.strip()) for m in x.split(",")]
            )
        )
    ...
    def analyze_multi_manager_favorites(self) -> pd.DataFrame:
        """Check if required columns exist."""
        required_columns = ["market_cap", "sector"]
        if self.stocks_df is None or self.stocks_df.empty:
            return pd.DataFrame()
        ...
        if missing_columns:
            logging.debug(
                f"Missing columns for analyze_multi_manager_favorites: {missing_columns}"
            )
            # Return empty DataFrame if essential columns are missing
            return pd.DataFrame()
    ...
    def analyze_stocks_by_price(self, max_price: float, label: str) -> pd.DataFrame:
        ...
        if "market_cap" not in grouped.columns and self.stocks_df is not None:
            ...
            grouped = grouped.merge(
                stock_info, left_index=True, right_on="ticker", how="left"
            )
    ...
    def analyze_stocks_by_market_cap(self, cap_category: str) -> pd.DataFrame:
        ...
        if missing_columns:
            logging.debug(
                f"Missing columns for analyze_stocks_by_market_cap: {missing_columns}"
            )
            # Return empty DataFrame if essential columns are missing
            return pd.DataFrame()
        ...
        analysis_df["cap_category"] = analysis_df["market_cap"].apply(
        ...
        # Format market cap
        grouped["market_cap_display"] = grouped["market_cap"].apply(
            self.format_market_cap
        )
    ...
    def analyze_52_week_low_buys(self) -> pd.DataFrame:
        """Check if required columns exist."""
        required_columns = ["52_week_low", "market_cap", "sector"]
        if self.stocks_df is None or self.stocks_df.empty:
            return pd.DataFrame()
        ...
        if missing_columns:
            logging.debug(
                f"Missing columns for analyze_52_week_low_buys: {missing_columns}"
            )
            # Return empty DataFrame if essential columns are missing
            return pd.DataFrame()
        ...
        if self.holdings_df is not None:
            ...
            buy_summary = buy_summary.merge(
                current_data, left_index=True, right_index=True, how="left"
            )
    ...
    def analyze_52_week_high_sells(self) -> pd.DataFrame:
        """Check if required columns exist."""
        required_columns = ["52_week_high", "market_cap", "sector"]
        if self.stocks_df is None or self.stocks_df.empty:
            return pd.DataFrame()
        ...
        if missing_columns:
            logging.debug(
                f"Missing columns for analyze_52_week_high_sells: {missing_columns}"
            )
            # Return empty DataFrame if essential columns are missing
            return pd.DataFrame()
    ...
    def analyze_most_sold_stocks(self) -> pd.DataFrame:
        ...
        if missing_columns:
            logging.debug(
                f"Missing columns for analyze_most_sold_stocks: {missing_columns}"
            )
            # Return empty DataFrame if essential columns are missing
            return pd.DataFrame()
        ...
        if self.holdings_df is not None:
            ...
            sell_analysis["remaining_holders"] = sell_analysis[
                "remaining_holders"
            ].fillna(0)
        ...
        # Add manager names
        sell_analysis["selling_manager_names"] = sell_analysis[
            "selling_managers"
        ].apply(
            lambda x: ", ".join(
                [self.manager_name_map.get(m.strip(), m.strip()) for m in x.split(",")]
    ...
    def analyze_contrarian_plays(self) -> pd.DataFrame:
        ...
        if missing_columns:
            logging.debug(
                f"Missing columns for analyze_contrarian_plays: {missing_columns}"
            )
            # Return empty DataFrame if essential columns are missing
            return pd.DataFrame()
        ...
        contrarian = contrarian.sort_values(
            "contrarian_score", key=abs, ascending=False
        )
    ...
    def analyze_concentration_changes(self) -> pd.DataFrame:
        ...
        change_summary = (
            significant_changes.groupby(["ticker", "manager_id"])
            .agg(
                {
        ...
        current_positions = self.holdings_df[
            ["ticker", "manager_id", "portfolio_percent", "value"]
        ]
        ...
        change_details = change_summary.groupby("ticker").apply(
            lambda x: ", ".join(
                [
                    ...
                    for _, row in x.nlargest(
                        3, "percentage_change", keep="all"
                    ).iterrows()
                ]
            )
        )
        ...
        current_holdings_only = ticker_summary[
            ticker_summary["total_value"] > 0.0
        ].copy()
        ...
        return self.apply_precision_formatting(
            current_holdings_only.reset_index()
        ).head(50)
    ...
    def analyze_sector_rotation(self) -> pd.DataFrame:
        ...
        if missing_columns:
            logging.debug(
                f"Missing columns for analyze_sector_rotation: {missing_columns}"
            )
            # Return empty DataFrame if essential columns are missing
            return pd.DataFrame()
        ...
        if self.history_df is not None:
            ...
            recent_activity["buy_actions"] = recent_activity[
                "activity_breakdown"
            ].apply(lambda x: x.get("Buy", 0) + x.get("Add", 0))
            recent_activity["sell_actions"] = recent_activity[
                "activity_breakdown"
            ].apply(
                lambda x: x.get("Sell", 0) + x.get("Reduce", 0) + x.get("Sold Out", 0)
            )
    ...
    def analyze_manager_performance_3year(self) -> pd.DataFrame:
        ...
        for manager_id in three_year_data["manager_id"].unique():
            ...
            manager_data_with_dates["date"] = pd.to_datetime(
                manager_data_with_dates["date"]
            )
            manager_periods = (
                manager_data_with_dates.groupby("date")["value"].sum().sort_index()
            )
        ...
        if not performance_df.empty:
            ...
            logging.info(
                f"Created 3-year manager performance analysis for {len(performance_df)} managers"
            )
    ...
    def analyze_quarterly_activity_timeline(self) -> pd.DataFrame:
        ...
        timeline_data = self.history_df[
            self.history_df["portfolio_impact"].abs() >= 1.0
        ].copy()
        ...
        for quarter in sorted(timeline_data["quarter"].unique()):
            ...
            major_buys = quarter_data[quarter_data["action_class"] == "buy"].nlargest(
                5, "portfolio_impact"
            )
            major_sells = quarter_data[quarter_data["action_class"] == "sell"].nlargest(
                5, "portfolio_impact"
            )
            ...
            manager_names = ", ".join(
                [
                    ...
                    for mid in quarter_data["manager_id"].unique()[
                        :10
                    ]  # Top 10 most active
                ]
            )
        ...
        if not timeline_df.empty:
            ...
            timeline_df = timeline_df.sort_values(
                ["year", "quarter_num"], ascending=False
            )
            ...
            logging.info(
                f"Created quarterly activity timeline for {len(timeline_df)} quarters"
            )
    ...
    def analyze_top_performing_plays(self) -> pd.DataFrame:
        ...
        for _, buy_row in major_buys.iterrows():
            ...
            if not sell_actions.empty:
                ...
                major_sell = sell_actions.loc[
                    sell_actions["portfolio_impact"].abs().idxmax()
                ]

                # Calculate holding period
                hold_days = (
                    pd.to_datetime(major_sell["date"]) - pd.to_datetime(buy_date)
                ).days
                ...
                estimated_return = (
                    ((sell_impact - buy_impact) / buy_impact * 100)
                    if buy_impact > 0
                    else 0
                )
            else:
            ...
            top_plays.append(
                {
                    ...
                    "portfolio_impact_sell": abs(major_sell["portfolio_impact"])
                    if not sell_actions.empty
                    else None,
                    "estimated_return_pct": estimated_return,
                    "buy_action": buy_row["action"],
                    "sell_action": major_sell["action"]
                    if not sell_actions.empty
                    else "Still Held",
                }
            )
        ...
        if not plays_df.empty:
            ...
            plays_df = plays_df.sort_values(
                "estimated_return_pct", ascending=False, na_position="last"
            )
            ...
            logging.info(
                f"Created top performing plays analysis for {len(plays_df)} major positions"
            )
    ...
    def _create_manager_performance_3year_chart(self, visuals_dir: Path) -> None:
        ...
        performance_file = (
            ...
            / "historical"
            / "manager_performance_3year_summary.csv"
        )
        ...
        fig.suptitle(
            "3-Year Manager Performance Analysis", fontsize=20, fontweight="bold"
        )
        ...
        bars = ax1.barh(
            top_performers["manager_name"],
            top_performers["total_return_pct"],
        ...
        bars1 = ax2.bar(
            x_pos - width / 2,
            top_performers["start_portfolio_value"],
            width,
            label="Start Value",
            alpha=0.8,
            color="lightcoral",
        ...
        bars2 = ax2.bar(
            x_pos + width / 2,
            top_performers["end_portfolio_value"],
            width,
            label="End Value",
            alpha=0.8,
            color="lightgreen",
        ...
        plt.tight_layout()
        plt.savefig(
            visuals_dir / "manager_performance_3year.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _create_quarterly_activity_chart(self, visuals_dir: Path) -> None:
        ...
        timeline_file = (
            Path(self.output_dir) / "historical" / "quarterly_activity_timeline.csv"
        )
        ...
        fig.suptitle(
            "Quarterly Market Activity Timeline (2007-2025)",
            fontsize=20,
            fontweight="bold",
        )
        ...
        df["date"] = pd.to_datetime(
            df[["quarter_year", "month", "day"]].rename(
        ...
        ax1.plot(
            df["date"], df["total_transactions"], marker="o", linewidth=2, markersize=4
        )
        ...
        ax3.fill_between(
            df["date"], df["avg_portfolio_impact"], alpha=0.3, color="green"
        )
        ...
        plt.tight_layout()
        plt.savefig(
            visuals_dir / "quarterly_activity_timeline.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _create_top_plays_chart(self, visuals_dir: Path) -> None:
        ...
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(20, 16))
        fig.suptitle(
            "Top Performing Investment Plays Analysis", fontsize=20, fontweight="bold"
        )

        # 1. Top 15 Best Returns
        top_winners = completed_plays.head(15)
        bars = ax1.barh(
            range(len(top_winners)),
            top_winners["estimated_return_pct"],
            color=plt.cm.RdYlGn(np.linspace(0.5, 0.9, len(top_winners))),
        )
        ax1.set_yticks(range(len(top_winners)))
        ax1.set_yticklabels(
            [
                f"{row['manager_name']}: {row['ticker']}"
        ...
        ax2.scatter(
            completed_plays["holding_days"],
            completed_plays["estimated_return_pct"],
            alpha=0.6,
            s=50,
            c=completed_plays["estimated_return_pct"],
            cmap="RdYlGn",
        )
        ...
        manager_stats = (
            completed_plays.groupby("manager_name")
            .agg({"estimated_return_pct": ["count", "mean"]})
            .round(1)
        )
        manager_stats.columns = ["total_plays", "avg_return"]
        manager_stats = manager_stats[
            manager_stats["total_plays"] >= 3
        ]  # Minimum 3 plays
        top_managers = manager_stats.nlargest(10, "avg_return")

        bars = ax3.bar(
            range(len(top_managers)),
            top_managers["avg_return"],
        ...
        ax3.set_title(
            "Top Manager Performance (Min 3 Plays)", fontsize=14, fontweight="bold"
        )
        ...
        completed_plays["return_bucket"] = pd.cut(
            completed_plays["estimated_return_pct"], bins=bins, labels=labels
        )
        ...
        plt.tight_layout()
        plt.savefig(
            visuals_dir / "top_performing_plays.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _create_performance_distribution_chart(self, visuals_dir: Path) -> None:
        ...
        fig.suptitle(
            "Manager Performance Distribution Analysis", fontsize=18, fontweight="bold"
        )
        ...
        ax1.axvline(
            df["total_return_pct"].mean(),
            color="red",
            linestyle="--",
            label=f'Mean: {df["total_return_pct"].mean():.1f}%',
        )
        ax1.axvline(
            df["total_return_pct"].median(),
            color="green",
            linestyle="--",
        ...
        top_means = [
            top_5["avg_position_impact"].mean(),
            top_5["volatility"].mean(),
            top_5["total_transactions"].mean(),
        ]
        bottom_means = [
            bottom_5["avg_position_impact"].mean(),
        ...
        ax3.set_title(
            "Top vs Bottom Performer Characteristics", fontsize=14, fontweight="bold"
        )
        ax3.set_xticks(x)
        ...
        outliers = df[
            (df["total_return_pct"] > df["total_return_pct"].quantile(0.9))
        ...
        plt.tight_layout()
        plt.savefig(
            visuals_dir / "performance_distribution.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def generate_all_reports(self) -> None:
        ...
        for max_price, label in price_analyses:
            analyses[label] = (
                lambda mp=max_price, l=label: self.analyze_stocks_by_price(mp, l)
            )
        ...
        for cap_category, label in cap_analyses:
            analyses[label] = lambda cc=cap_category: self.analyze_stocks_by_market_cap(
                cc
            )
        ...
        logging.info(
            "Analysis complete! Check the 'analysis' directory for current results and 'analysis/historical' for historical data."
        )
...
"""
    ...
    def analyze_historical_stock_performance(self) -> pd.DataFrame:
        ...
        for ticker in three_year_data["ticker"].unique():
            ...
            if not recent_data.empty:
                recent_buys = recent_data[recent_data["action_class"] == "buy"][
                    "portfolio_impact"
                ].sum()
                recent_sells = abs(
                    recent_data[recent_data["action_class"] == "sell"][
                        "portfolio_impact"
                    ].sum()
                )
            ...
            top_managers = (
                ticker_data.groupby("manager_id")["portfolio_impact"]
                .sum()
                .abs()
                .nlargest(3)
            )
            top_manager_names = [
                self.manager_name_map.get(mid, mid) for mid in top_managers.index
            ]
        ...
        if not performance_df.empty:
            ...
            performance_df = performance_df.sort_values(
                "net_activity_score", ascending=False
            )
            ...
            logging.info(
                f"Created stock performance analysis for {len(performance_df)} stocks"
            )
    ...
    def _create_comprehensive_holdings_chart(self) -> None:
        ...
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(20, 16))
        fig.suptitle(
            "Comprehensive Holdings Analysis Overview", fontsize=20, fontweight="bold"
        )

        # 1. Top Holdings by Value (Bubble Chart)
        ticker_summary = (
            self.holdings_df.groupby("ticker")
            .agg({"value": "sum", "manager_id": "nunique", "portfolio_percent": "mean"})
            .reset_index()
        )
        ...
        scatter = ax1.scatter(
            top_holdings["manager_id"],
            top_holdings["value"] / 1e9,
            s=top_holdings["portfolio_percent"] * 20,  # Size by avg portfolio %
            alpha=0.6,
            c=top_holdings["value"] / 1e9,
            cmap="viridis",
        )
        ...
        ax1.set_title(
            "Top Holdings: Value vs Manager Count\n(Bubble size = Avg Portfolio %)",
            fontsize=14,
            fontweight="bold",
        )
        ax1.grid(alpha=0.3)
        ...
        manager_summary = (
            self.holdings_df.groupby("manager_id")
            .agg({"value": "sum", "ticker": "nunique"})
            .reset_index()
        )
        manager_summary["manager_name"] = manager_summary["manager_id"].map(
            self.manager_name_map
        )
        ...
        manager_summary["size_bucket"] = pd.cut(
            portfolio_sizes, bins=bins, labels=labels
        )
        ...
        ax2.set_title(
            "Manager Portfolio Size Distribution", fontsize=14, fontweight="bold"
        )

        # 3. Portfolio Concentration Analysis
        manager_summary["avg_position_size"] = (
            manager_summary["value"] / manager_summary["ticker"]
        )
        ...
        ax3.set_yticklabels(
            [f"{row['manager_name']}" for _, row in top_managers.iterrows()],
            fontsize=10,
        )
        ax3.set_xlabel("Number of Unique Stocks", fontsize=12)
        ax3.set_title(
            "Portfolio Diversification (Top 15 Managers)",
            fontsize=14,
            fontweight="bold",
        )
        ...
        ax4.hist(
            all_position_values, bins=50, alpha=0.7, color="skyblue", edgecolor="black"
        )
        ...
        ax4.axvline(
            all_position_values.mean(),
            color="red",
            linestyle="--",
            label=f"Mean: ${all_position_values.mean():.1f}M",
        )
        ax4.axvline(
            all_position_values.median(),
            color="green",
            linestyle="--",
        ...
        ax4.grid(alpha=0.3)
        ax4.set_xlim(
            0, np.percentile(all_position_values, 95)
        )  # Remove outliers for better view

        plt.tight_layout()
        plt.savefig(
            self.visual_dir / "comprehensive_holdings_overview.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _create_enhanced_manager_chart(self) -> None:
        ...
        ax1.set_title(
            "Manager Risk Profile\n(Size = Total Value, Color = Stock Count)",
            fontsize=14,
            fontweight="bold",
        )
        ax1.grid(alpha=0.3)
        ...
        ax2.scatter(
            manager_data["stock_count"],
            manager_data["max_position_pct"],
            alpha=0.6,
            s=60,
            c=manager_data["total_value"] / 1e9,
            cmap="plasma",
        )
        ax2.set_xlabel("Number of Stocks", fontsize=12)
        ax2.set_ylabel("Largest Position (%)", fontsize=12)
        ax2.set_title(
            "Portfolio Concentration Strategy", fontsize=14, fontweight="bold"
        )
        ax2.grid(alpha=0.3)
        ...
        ax3.set_title(
            "Top 15 Managers by Portfolio Value", fontsize=14, fontweight="bold"
        )
        ax3.grid(axis="x", alpha=0.3)
        ...
        for i, value in enumerate(top_15["total_value"] / 1e9):
            ax3.text(
                value + max(top_15["total_value"] / 1e9) * 0.01,
                i,
                f"${value:.1f}B",
                ha="left",
                va="center",
        ...
        manager_data["size_category"] = pd.cut(
            manager_data["total_value"] / 1e9,
        ...
        wedges, texts, autotexts = ax4.pie(
            size_categories.values,
            labels=size_categories.index,
            autopct="%1.1f%%",
            startangle=90,
            colors=["lightcoral", "orange", "lightgreen", "darkgreen"],
        )
        ax4.set_title(
            "Manager Categories by Portfolio Size", fontsize=14, fontweight="bold"
        )

        plt.tight_layout()
        plt.savefig(
            self.visual_dir / "enhanced_manager_analysis.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def _create_market_dynamics_chart(self) -> None:
        ...
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(20, 16))
        fig.suptitle(
            "Market Dynamics and Investment Trends", fontsize=20, fontweight="bold"
        )
        ...
        scatter = ax1.scatter(
            top_tickers["manager_count"],
            top_tickers["total_value"] / 1e9,
            s=top_tickers["avg_portfolio_pct"] * 15,  # Size by avg allocation
            alpha=0.6,
            c=top_tickers["max_portfolio_pct"],
            cmap="Reds",
        )
        ...
        ax1.set_ylabel("Total Value (Billions $)", fontsize=12)
        ax1.set_title(
            "Stock Popularity vs Total Investment\n(Size = Avg %, Color = Max %)",
            fontsize=14,
            fontweight="bold",
        )
        ax1.grid(alpha=0.3)
        ...
        for manager_id in self.holdings_df["manager_id"].unique():
            manager_holdings = self.holdings_df[
                self.holdings_df["manager_id"] == manager_id
            ]
            ...
            if total_value > 0:
                ...
                hhi_data.append(
                    {
                        "manager_id": manager_id,
                        "manager_name": self.manager_name_map.get(
                            manager_id, manager_id
                        ),
                        "hhi": hhi,
        ...
        ax2.axhline(
            2500, color="red", linestyle="--", alpha=0.7, label="Highly Concentrated"
        )
        ax2.axhline(
            1500,
            color="orange",
        ...
        bars = ax3.barh(
            range(len(conviction_plays)),
            conviction_plays["portfolio_percent"],
            color=plt.cm.Reds(np.linspace(0.4, 0.9, len(conviction_plays))),
        )
        ax3.set_yticks(range(len(conviction_plays)))
        ax3.set_yticklabels(
            [
                f"{row['ticker']} ({self.manager_name_map.get(row['manager_id'], row['manager_id'])[:15]})"
                for _, row in conviction_plays.iterrows()
            ],
            fontsize=9,
        )
        ...
        if (
            ...
            holdings_with_mcap = holdings_with_mcap[
                holdings_with_mcap["market_cap"].notna()
            ]
            ...
            if not holdings_with_mcap.empty:
                ...
                holdings_with_mcap["mcap_bucket"] = pd.cut(
                    mcap_billions, bins=bins, labels=labels
                )

                # Calculate total investment by market cap bucket
                mcap_investment = holdings_with_mcap.groupby("mcap_bucket")[
                    "value"
                ].sum()

                wedges, texts, autotexts = ax4.pie(
                    mcap_investment.values,
                    labels=mcap_investment.index,
                    autopct="%1.1f%%",
                    startangle=90,
                )
                ax4.set_title(
                    "Investment Allocation by Market Cap",
                    fontsize=14,
                    fontweight="bold",
                )
            else:
        ...
        plt.tight_layout()
        plt.savefig(
            self.visual_dir / "market_dynamics_analysis.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
